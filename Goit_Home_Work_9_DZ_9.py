...
#            Вся теорія вище нажаль невикористовувалсь. Можливо в наступних завданнях.

# **************************Рішення завдання 6 модуля 9 від одногрупника (Василь Єгупов) ********************

##   =================================   Алгоритм :  ==========================================================

# *generator_numbers отримує стрічку тип *str, в якій є цілі числа, шукає перше, викидає з стрічки все з початку до знайденого числа включно, 
# повертає знайдене число і чекає наступного виклику. І так в циклі, поки в стрічці є числа. 
# Далі, в *parser_str передаємо  генератор чисел із строкою, яку треба розібрати, 
# і в циклі отримуємо числа з генератора методом next(),
# поки не виникне виключення StopIteration (генератор вже не може нічого видати, бо в строке закінчилися числа). 


import re # імпортуєм бібліотеку *re .
def generator_numbers(string=""):
    '''Допоміжна функція *генераор .Функція що приймає один аргумент - стрічку, в якій будемо шукати наші числові значення  методом re.search(*patern, *str).
    Метод *re.search(*patern, *string(тип_*str)) -  метод , шукає в стрітчці *string значення зміної *patern , 
                                                  # повертає перше знайдене співпадіння в обєкт типу *re.Match_object,
                                                  # щоб потім діставати потрібні значення з обкту *re.Match_object за допомогою вбудованих функції для роботи з цим обєктом таких як *.group(), *.end() та інших  )
                                                  # Якщо співпадінь немає то повертає *None 
    Оскільки *re.search повертає тільки перше знайдене значення , то цикл потрібний для пошуку наступного . Пошук буде відбуватись за допомогою циклу *while ,. В циклі на кожній ітерації  , 
    знайдене числове значення, повертаємо кожен раз з функції ітератором *yield . *yield - повертає відразу  поточний результат виконання функції *generator_numbers(string=""),
    при зверненні до неї. *generator_numbers(string="") перший раз, поверне перше знайдене значення(в нашому випадку шукаємо числове значення в стрітці) і призупинить виконання циклу *while до моменту поки ще раз не звернуться до функції  ,
    При наступному зверненні до функції  запуститься наступна ітерація циклу і так поки не виконається умова завершення циклу(в нашому випадку . коли закінчаться числові значення в стрічці , тобто *result прийме значення *None) 
    Ітератор *yield - працює як вубудована функwія *return Відміність в тому що *return завжди завершує виконання функції а *yield лишень призупинняє .
    і при наступному зверненні шукає наступний результат, а не починає виконання функції з початку. Відповідно в памяті залишається поточні значення всіх зміних і функції на момент зупинки.
    Тому повторний процес пошуку відбувається вже зврахуванням поточних значень зміних і функції.
    продовжуємо пошук нового числового значення поки не закінчиться стрічка
     тобто коли *re.search(*patern, *string) поверне результат *None, Тоді цикл *while з аргументом *result == *None припинеться  '''
    result = re.search(r"\d+", string) # Перше знайдене значення, оскільки ми будемо обрізати нашу стрічку і передавати на початок циклу вже без першого значення , то на наступному кроці, поточне перше значення буде наступним по факту.  
                                       # Де *patern == r"\d+" (регулярний вираз що означає всі цілі числові значення), *string - наша стрічка в якій шукаємо числові значення .
                                       # re.search - завжди повертає перше знайдене значення в обєкт *re.Match_object , тому нам буде порібно обрізати нашу стрічку відразу після цього значення і повертати на повторний пошук вже обрізану.
                                       # Для нашого випадку перше значення ; *result = *<re.Match object; span=(58, 61), match='100'>

    #print(result) # Допоміжний принт 

    while result: # Цикл, умова завершення *result == *None 

        string = string[result.end():] # Виконуємо зріз стрічки *string , string[*індекс_елемента_з_якого_почати_зріз : *індекс_елемента_яким_завершити_зріз(невключає_цей_елемент)] 
                                       # В нашому випадку після першого виконання ; *string -== *", from the northern colonies $500, and the king gave $1000."
                                       # а *result = "100"
                                       # string[result.end():] , де # *string - стрічка в якій виконуємо зріз, 
                                                                    # *result.end() метод обєкту *re.Match_object , який повертає індекс символа , який знаходиться відразу після *patern.
                                                                    # *result.end()- , поверне індекс з якого будемо починати зріз.#Початок зрізу.
                                                                    #  Якщо кінець зрізу не вказаний , то це означає ,що до кінця стрічки включно з останім символом.
                                                                    #  Якщо не вказаний початок зрізу , то це означає , що зріз починається з першого символа (0-індекс) 

        #print(string) # Допоміжний принт

        yield int(result.group()) # Ітератор *yield поверне з функції, перше знайдене значення і буде зберігати в памяті  стан кожної зміної на момент повернення , 
                                  # При наступному зверненні поверне наступне знайдене значення і в памяті будуть збережено настіпні занчення зміних на момент повернення.
                                  # буде повертати з функції поточне значення а в память записувати стан зміних на момент повернення.
                                  # Значення що будемо повертати . int(result.group()) 
                                            # Де *result.group() - це елемент списку який знайдений функцією *re.search(r"\d+", string)
                                            # *.group() - метод обєкту *re.Match_object , який повертає з обєкту значення що міститься в параметрі  *match .
                                            # Для нашого прикладу на першому кроці *result = *<re.Match object; span=(58, 61), match='100'>
                                            # Отже поверне "100"(тип *str) . 
                                            # Далі функція вбудована функція *int(*str) перетворить стрічку *"100" в число *100  
                                            # Отримане поточне значення і повертаємо з функції *generator_numbers(string="") ітератором *yield , а всі поточні стани зміних ,на момент повернення , в память інтерпритатора.  
                                            # Після повернення цикл *while (при повторному зверненні до функції ) почнеться з наступної після ітератора *yield рядка нашого коду а не спочатку.
        
        result = re.search(r"\d+", string)  # При повторному зверненні до функції цикл *while почнеться з цього рядка коду. Ця умова забезпечує вихід з циклу.
                                            # Оскільки умова виходу з циклу *while є *result то на початок циклу потрібно повернути наступне від поточного значення *result.
                                            # Коли наступне значення *result від поточного значення стане *None цикл завершиться повністю.

# def sum_profit(string):
#     '''Основна фукція , яка приймає одни аргумент, стрічку в якій будемо шукати числові значення за допомогою допоміжної функції генератора *generator_numbers(string), і сумувати їх.
#     Певерне суму всіх числових значеннь з стрічки тип *int '''
    
#     summa = 0 # Зміна в яку до поточного значення, будемо додавати наступне повернене з *generator_numbers(string) # Початкове значення 0.
#     parser_str = generator_numbers(string) # *parser_str- зміна яка буде містити почтоне значення геератора *generator_numbers(string).
#     while True: # цикл в якому , на кожній ітерації , будемо звертатись до генератора і отримувати поточне значення його значення.
#         # код написаний нижче . буде за допомогою функції *next(*зміна) ,повертати поточний результат з функції *generator_numbers(string) в зміну *number
#         # до спрацювання виключення(помилки) *StopIteration-(помилка яка виникає коли *next(*зміна) немає що повернути)
#         # Коли спрацює виклкюченя *StopIteration - *summa - буде містити суму всіх числових значень з нашого рядка *string 
#         try:   
#             number = next(parser_str) # на першому кроці *number == 100 для нашого прикладу. На останьому коці виникне помилка *StopIteration , *return поверне значення *summa і цикл припиниься.
#         except StopIteration: # Виключення яка припиняє поточний цикл , якщо в *next(parser_str) немає значень для повернення 
#             return summa # На момент припинення циклу *summa == 1600 , для нашого прикладу. 
#         else:            # Умова , поки невинекне виключення до поточного значення *summa додавати поточне значення *number
#             summa += number 
# Примітка : # Якщо нам не потрібно нічого , додатково , робити з проміжними значеннями які повернуті з генератора ,то краще застосувати конструкцію з циклом for без функції *next(*зміна)
             # нижче код більш правильний саме для нашого випадку , оскільки нам не потрібно нічого доатково робити з поточним значенням генератора
def sum_profit(string): # Більш правильний код оголошення функції *sum_profit(string) для  конкретного нашого випадку , але він недає повного розуміння переваг ітератора *yield
    ''' Основна функція , приймає один аргумент, стрічку в якій будемо шукати числові значення
      і  в циклі *for звертатись до нашого генератора generator_numbers(string) на кожній ітерації циклу.
       Повернуте поточне значення записувати в *parser_str і втілі циклу додавати поточне значення *parser_str до поточного значення *summa  '''
    
    summa = 0 # зміна в яку будемо дадавати значення. # Початкове значення 0.

    mygenerator = generator_numbers(string) # змніа в якій буде міститись повернуте поточне значення з *generator_numbers(string)

    for parser_str in mygenerator : # Цикл в якому на кожній ітерації *parser_str буде отримувати значення з поточного значення *mygenerator  
                                    # Допоки в *mygenerator не закінчаться значення повернуті з *generator_numbers(string)
                                    # Для нашого значення *string , # на першй ітерації *parser_str = 100
                                                                    # на другії - *parser_str = 500
                                                                    # на третії(останії для нашого випадку)  *parser_str = 1000
        summa += parser_str # До поточної суми дадаєм поточне значення.
    return summa # Повертаємо остаточну суму з основної функції.
   
# #**************************** Моє вирішення ,без замикань ітераторів і оператора yield . автоперевірка приняла без зауважен.*************************
...
string = "The resulting profit was: from the southern possessions $ 100, from the northern colonies $500, and the king gave $1000."  # Тестове значення .
